...
import { Modal,ModalHeader,ModalBody,ModalFooter,Form,Row,Col,Button,Table,Input } from 'reactstrap';
import PropTypes from 'prop-types'
import api from '../../constants/api';
import message from '../Message';

function DeliveryOrderEditModal({
    deliveryOrderEditModal,setDeliveryOrderEditModal,deliveryOrderId
}) {
...
const editDeliveryProducts=()=>{
  deliverOrderProducts.forEach((el)=>{
    api.post('/purchaseorder/editDelieryOrderHistory',el)
    .then(() => {
...
setDeliveryOrderProducts(copyDeliverOrderProducts);
  ...
  <Modal size='xl' isOpen={deliveryOrderEditModal} >
    <ModalHeader >   Edit Delivery Order
                <Button color="secondary" onClick={()=>{setDeliveryOrderEditModal(false)}}> X </Button>
                </ModalHeader>
    <ModalBody>
      <Row>
        <Col md="12">
  ...
  <Row>
          ...
          deliverOrderProducts.map((element,index) => {
            return (
              ...
              <tr key={element.delivery_order_id}>
                ...
                <td data-label="title"><Input type='text' name='title' value={element.item_title}/></td>
                <td data-label="quantity"><Input type="text" name='quantity' value={element.quantity}  onChange={(e)=>updateState(index,"quantity",e)}/></td>
                <td data-label="status"><Input
                    type="select"
                ...
                <td data-label="remarks"><Input type='text' name='remarks' value={element.remarks} onChange={(e)=>updateState(index,"remarks",e)}/></td>
    ...
    <Button
        ...
        color="primary"
        onClick={()=>{editDeliveryProducts();setDeliveryOrderEditModal(false)}}
      >
        Submit
      </Button>
      <Button
