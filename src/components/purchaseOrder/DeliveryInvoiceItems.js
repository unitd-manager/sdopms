...
const ProjectWorksheet = ({ WorkSheet, getDeliveryInvoiceById }) => {
  ...
  const getLineItem = (taskId) => {
    api
      .post('/purchaseorder/getDeliveryInvoicehistory1', { delivery_invoice_id: taskId })
      .then((res) => {
          ...
          return (
            <Row>
              ...
              <Col>
                <FormGroup>
                  <span>
                    {element.invoice_date ? moment(element.invoice_date).format('DD-MM-YYYY') : ''}
                  </span>
                </FormGroup>
              </Col>
              ...
              {showPcItemsTable && selectedInvoiceId === element.delivery_invoice_id && (
                <table className="lineitem2 border border-secondary rounded"  >
                  <thead>
                    <tr>
