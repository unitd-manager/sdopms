...
const PdfDeliveryOrder = ({deliverOrderId}) => {
  ...
  const getPoProduct = () => {
    api
      .post('/purchaseorder/getProjectDeliveryOrderByPdf', { delivery_order_id: deliverOrderId })
      .then((res) => {
      ...
      content: [
        ...
        '\n',
        {
          ...
          ${deliveryData.billing_address_country? deliveryData.billing_address_country:''}\n`,
            ...
            { text: `DATE :${(deliveryData.date)? moment(deliveryData.date).format('DD-MM-YYYY'):''}`, style: ['textSize'] },
          ],
        },
        '\n',
